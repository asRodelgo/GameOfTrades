...
barplotPlayerStats <- reactive({
  barplot_PlayerStats(values$playersDatabase, values$playersRanks, input$playerName)
})
...
```{r}
similar <- reactive({
  similar_players(values$playersDatabase, values$playersTSNE, input$playerName)
})
...
tsne_predicted <- reactive({
  plot_tsne_predicted(values$playersTSNE, input$player_num_clusters, input$playerName)
})
...
dateInput('regSeasonDay', 'Select a day:', min=datesRange[1], max = datesRange[length(datesRange)],startview = 'month', value = ifelse(datesRange[length(datesRange)] > Sys.Date(), datesRange[length(datesRange)], ifelse(datesRange[1] > Sys.Date(), datesRange[1],Sys.Date())))
...
observeEvent(input$tradeButton, {
  ...
  isolate({
    ...
    playersPredicted2 <- merge(playersPredicted_Off_Def,values$playersAdjMin[,c("Player","Exp","Age","Tm","effMin")], by ="Player") %>%
    ...
    playersNewPredicted_Final_adjMinPer2 <- select(values$playersDatabase, -contains("Per"), -effFG, -effFGA, -effPTS, -effTRB)
    teamsPredicted <- .teamsPredictedPower_2018(data = playersNewPredicted_Final_adjMinPer2)
